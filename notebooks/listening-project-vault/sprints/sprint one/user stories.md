# user stories
## pick rate
- as a music lover, i want to be able to analyze my [[product goal#first time pick rates|first time pick rates]] for my last 50 plays (the limit of the api's available history) so that i can make sense of my preferences.
...
# edge cases
if the user has no listening data then in the results just say "no data to analyze" (this is the only 
if there is no "previous song" then the current song will be considered the first pick of the session
if needed data is missing, we will ignore the pick and try and find the next
if zero picks are detected the user will get a message stating so and prompting them to retry because it's likely an error.